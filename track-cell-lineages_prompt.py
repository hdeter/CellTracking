...
#rootdir should be the directory that contains your original images from the working directory
#rootdir = 'CROPPED/'
rootdir = getdirname('Enter the path to the directory containing the images relative to the working directory (e.g. Aligned): ') 
rootdir = rootdir + '/'


#the number for the first frame in the dataset
FIRSTFRAME = int_input('Enter the number of the first frame in the dataset (e.g. 448): ')
#used to test every nth image for overlap.
FRAMESKIP = 1
#the number of the last frame in the dataset
#FRAMEMAX = 467
FRAMEMAX = int_input('Last frame in the dataset (e.g. 467): ')
...
#dPeriodFL = 10  # frames
dPeriodFL = int_input('Number of frames between fluorescence images (i.e. every nth image; e.g. 10): ')
...
#mask image
#filefmt = 'Mask2/20171212_book-p-%03d_cropped_mask.png'
filefmtB = False
while not filefmtB:
	filefmt = text_input('Enter filenames of the mask files preceding the filenumbers (relative to image directory) (e.g. Mask2/20171212_book-p): ')
	filefmt = filefmt + '-%03d.png'
	filefmtB = os.path.isfile(rootdir + (filefmt % FLINITIAL))
	if not filefmtB:
		print ('cannot find file ' + filefmt % FLINITIAL)


#phase image
#filefmt2 = '20171212_book-p-%03d_cropped.tif'
filefmt2B = False
while not filefmt2B:
	filefmts = text_input('Enter filenames of the original images preceding channel and filenumber (e.g. 20171212_book): ')
	filefmt2 = filefmts + '-p-%03d.tif'
	filefmtfl = filefmts + '-%s-%03d.tif'
	filefmttest = filefmtfl % ('p', FLINITIAL)
	filefmt2B = os.path.isfile(rootdir + filefmttest)
	if not filefmt2B:
		print ('cannot find file ' + filefmt2 % (FLINITIAL))


#name to save labeled stuff
#labelname = 'Lineages/20171212_book-p-%03d_lineage.png'
labelname = text_input('Enter filenames to save labeled images as (e.g. 20171212_book-lineages): ')
labelname = labelname + '-%03d.png'

#output name for cell statistics csv
#cellstatCSV = 'global_cell_statistics.csv'
cellstatCSV = text_input('Enter filename for cell statistics csv (e.g. global_cell_statistics): ') + '.csv'
#output name for lineages csv
#lineageCSV = 'lineagedata.csv'
lineageCSV = text_input('Enter filename for lineage statistics csv (e.g. lineagedata): ') + '.csv'
...
#useful if you want to visualize cell and compare to output
...
writeLabels = True
#image to write labels on (currently set to phase images, can use mask images)
labelimage = filefmt

# radius of how far CoM's can be and still check distances
THRESHOLD = 150
