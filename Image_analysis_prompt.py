...
#Change this cell depending on desired outcome of run (follow the comments)
#Also change cell 3 to affect video rendering (change colors, skip any XY regions, etc.)
#if rendering plots an 'out' directory is required in the root directory

#Set to the name of the experiment directory as a string (ex: 'practice')
#directory should be in working directory (the directory the script is in)
#EXPT_NAME = 'CROPPED'
EXPT_NAME = getdirname('Name of experiment directory (e.g. Aligned): ')
print
...
#b_ANALYZE = True
b_ANALYZEB = False
while not b_ANALYZEB:
	b_ANALYZE = bool_input('Do you want to analyze the files (Y/N): ')
	if not b_ANALYZE:
		b_ANALYZEB = getfilename(EXPT_NAME_PKL)
		if not b_ANALYZEB:
			print ('analyzing the files is required to render videos')
	else:
		b_ANALYZEB = True

#set to the name of the ROI file for analysis as a string (ex: 'ROI_file'); if there is no ROI file set to None
#ROI file is XY#,BX,BY,W,H in pixels
#ROIFILE = 'ROI_measure.csv'
...
#b_RENDER = True
b_RENDER = bool_input('Do you want to render the files (Y/N): ')
print

#If you have masks and want to contour the image
#~ MaskDirectory = 'CROPPED/Mask2'
#~ ContourImage = False
ContourImage = bool_input('Do you want to contour cells based on masks (Y/N): ')
if ContourImage:
	MaskDirectory = getdirname('Enter the path to the directory containing masks relative to the working directory(e.g. Aligned/Mask2): ')
		
#crops the image based on csv file 
#~ CroptoROI = True
#~ ROICROPFILE = 'ROI_measure.csv'
CroptoROI = bool_input('Do you want to crop the images based on an ROI (Y/N): ')
if CroptoROI:
	ROICROPFILE = getfilename('Enter the path (relative to the working directory) to the csv file for the ROI (e.g. ROI.csv): ')

#picklefile with lineage text (output by lineagetracking.py)
...
if Writelineagetext:
	Lineagetextfile = getfilename('Enter the path (relative to the working directory) to the pickle file containing lineage data (e.g. folder/lineagetext.pkl): ')

# Set to number of channels, including phase
CMAX = 2
print

#first frame that has fluorescence data
#FRAMEMIN = 9
FRAMEMIN = int_input('First frame with fluorescence image (e.g. 9): ')

#number of frames between fluorescence data
#frameSkip = 10
frameSkip = int_input('Number of frames between fluorescence images (i.e. every nth image): ')
# parameters for images
#
MaxC = 1                                # max number of C channels in image data
#AIfmtInFile = '20171212_book-%s-%03d_cropped.tif'       # format string for data
AIfmtInFileB = False
while not AIfmtInFileB:
	AIfmtInFile = text_input('Name of image files preceding channel and filenumber (e.g. 20171212_book): ')
	AIfmtInFile = AIfmtInFile + '-%s-%03d.tif'
	try:
		AIfmtInFileB = os.path.isfile(EXPT_NAME + '/' + AIfmtInFile % ('p',FRAMEMIN))
	except:
		print ('cannot format files')
	if not AIfmtInFileB:
		print ('cannot find file: ' + AIfmtInFile % ('p',FRAMEMIN))
		

imgScale = 1                                 # scale to import images

#framerate for final video. Start with 10
FRAMERATE = 10
...
if (b_RENDER):
    ...
    timePerFrame = float_input('What is the time per frame in minutes (e.g. 0.5): ')
    #timePerFrame = (15.0/60.0);
    ...
    CONFIGVARSimg_label[2] = 'GFP signal'
    CONFIGVARSimg_label[3] = 'Channel 3'
    CONFIGVARSimg_label[4] = 'Channel 4'
